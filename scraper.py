...
def getBook(url, db):
	...
	try:
		...
		rating_count = int(rating.replace(",",""))
	...
	for book in books:
		try:
			...
			if book["book_id"] < book_id:
				db_book.update_one({"book_id" :	book["book_id"]}, abook)
			else:
	...
	for author in authors:
		try:
			...
			if author["author_id"] < author_id:
				db_author.update_one({"author_id" : author.author_id}, {
					"$set"
				})
			else:
	...
	while True:
		...
		if choose == "1":
			...
			if choose2 == "2":
				id = input("enter id: ")
				if choose3 == "1":
					info = dict(input("enter data in dict format:"))
			...
			if choose2 == "3":
				...
				if choose3 == "1":
					info = dict(input("enter data in dict format:"))
					r =  requests.put("http://localhost:5000/api/book", data=json.dumps(info),  headers={'Content-Type': 'application/json'})
					print(r.json())
				if choose3 == "2":
					info = dict(input("enter data in dict format:"))
					r =  requests.put("http://localhost:5000/api/author", data=json.dumps(info),  headers={'Content-Type': 'application/json'})
					print(r.json())
			if choose2 == "4":
				id = input("enter id: ")
				if choose3 == "1":
